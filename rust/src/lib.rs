// https://github.com/parcel-bundler/lightningcss/blob/master/c/src/lib.rs
...
#[repr(C)]
pub struct CssModuleReference {
  /// The local (compiled) name for the reference, or
  /// The referenced global name, or
...
impl From<lightningcss::css_modules::CssModuleReference> for CssModuleReference {
  fn from(reference: lightningcss::css_modules::CssModuleReference) -> Self {
    ...
    match reference {
      Local { name } => CssModuleReference { name: name.into(), specifier: Default::default() },
      Global { name } => CssModuleReference { name: name.into(), specifier: Default::default() },
      Dependency { name, specifier } => CssModuleReference {
        name: name.into(),
